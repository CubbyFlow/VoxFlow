...
class DecisionMaker
{
 public:
    ...
    [[nodiscard]] static VkResult pickLayers(std::vector<const char*>& used, const std::vector<VkLayerProperties>& properties,
                                             const std::vector<Context::Entry>& requested);

    [[nodiscard]] static VkResult pickExtensions(std::vector<const char*>& used, const std::vector<VkExtensionProperties>& properties,
