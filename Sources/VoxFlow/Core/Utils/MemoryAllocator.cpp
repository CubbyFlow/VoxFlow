...
BlockAllocator::BlockAllocator(const bool isThreadSafe) : _isThreadSafe(isThreadSafe)
{
}
...
LinearBlockAllocator::LinearBlockAllocator(const uint64_t totalSize, const bool isThreadSafe) : BlockAllocator(isThreadSafe), _totalSize(totalSize)
{
...
void LinearBlockAllocator::deallocateInner(const uint64_t offset, const uint64_t size)
{
    (void)offset;
    (void)size;
}
...
LinearMemoryAllocator::LinearMemoryAllocator(const uint64_t totalSize, const bool isThreadSafe) : _linearBlockAllocator(totalSize, isThreadSafe)
{
...
{
    const bool isInThisAllocator = (static_cast<uint8_t*>(_dataAddress) <= static_cast<uint8_t*>(data)) &&
                                   (static_cast<uint8_t*>(_dataAddress) < (static_cast<uint8_t*>(_dataAddress) + size));
    ...
    {
        const uint64_t offset = static_cast<uint64_t>(static_cast<uint8_t*>(data) - static_cast<uint8_t*>(_dataAddress));
