...
namespace VoxFlow
{
bool SceneObjectLoader::loadSceneObject(const std::filesystem::path& gltfPath, ResourceUploadContext* asyncUploadContext, CommandStream* asyncCmdStream,
                                        SceneObjectCollection* outCollection)
{
    ...
    std::unordered_map<std::string, tf::Task> loadTasks;

    loadTasks.emplace("LoadBuffers", taskflow
                                         .emplace([this, &buffers, &asset]() {
    ...
    loadTasks.emplace(
        "LoadImages",
        taskflow
            .emplace([this, &textureUploadInfos, &asset]() {
                    ...
                    std::visit(
                        [&textureUploadInfos, &asset](auto&& arg) {
                            ...
                            if constexpr (std::is_same_v<T, fastgltf::sources::URI>)
                            {
                                ...
                                textureUploadInfos.push_back({ .uploadDstTexture = nullptr,
                                                               .textureRawData = data,
                                                               .extent = glm::vec2(width, height),
                                                               .numChannels = nrChannels,
                                                               .numBytes = width * height * nrChannels * sizeof(uint8_t)});
                            }
                            else if constexpr (std::is_same_v<T, fastgltf::sources::Vector>)
                            {
                                int width, height, nrChannels;
                                unsigned char* data =
                                    stbi_load_from_memory(arg.bytes.data(), static_cast<int>(arg.bytes.size()), &width, &height, &nrChannels, 4);
    ...
    loadTasks.emplace("LoadMaterials", taskflow
                                           .emplace([this, &materials, &asset]() {
    ...
    loadTasks.emplace("UploadTextures", taskflow
                                            .emplace([this, &textureUploadInfos, asyncUploadContext, asyncCmdStream]() {
                                                for (auto& textureUploadInfo : textureUploadInfos)
                                                {
                                                    asyncUploadContext->addPendingUpload(UploadPhase::Immediate, textureUploadInfo.uploadDstTexture,
                                                                                         {
                                                                                             ._data = textureUploadInfo.textureRawData,
                                                                                             ._size = textureUploadInfo.numBytes,
                                                                                             ._dstOffset = 0,
                                                                                         });
                                                }

                                                asyncUploadContext->processPendingUploads(UploadPhase::Immediate, asyncCmdStream);

                                                FenceObject fenceObject = asyncCmdStream->flush(nullptr, nullptr, false);

                                                // TODO(snowapril) :
                                                while (fenceObject.isCompleted() == false)
                                                {
                                                    std::this_thread::sleep_for(std::chrono::milliseconds(1));
                                                }
                                            })
                                            .name("UploadTextures")
                                            .succeed(loadTasks["LoadImages"]));
