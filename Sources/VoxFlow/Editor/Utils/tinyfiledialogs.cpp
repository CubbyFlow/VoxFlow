                             ...
                             int const aDefaultButton)      /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
    ...
    if (((aDialogType && strcmp("yesnocancel", aDialogType) && strcmp("okcancel", aDialogType) && strcmp("yesno", aDialogType))) || (lBoxReturnValue == IDOK) ||
                                  ...
                                  char const *const aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) && (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            p = saveFileDialogWinGui8(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription);
        }
        else
        {
            p = saveFileDialogWinGuiA(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription);
        }
    }
    else
#endif /* TINYFD_NOLIB */
                                  ...
                                  int const aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) && (!getenv("SSH_CLIENT") || getenv("DISPLAY")))
    {
        ...
        if (tinyfd_winUtf8)
        {
            p = openFileDialogWinGui8(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription,
                                      aAllowMultipleSelects);
        }
        else
        {
            p = openFileDialogWinGuiA(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription,
                                      aAllowMultipleSelects);
        }
    }
    else
